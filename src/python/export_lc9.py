...
def optical_fusion(bbox, start_time, end_time, target_ic):
    ...
    lc9_ds = hf.Landsat9(geometry, start_time, end_time)
    lc9_ds.collection = (
        lc9_ds.collection
    ...
    logging.info(f"Number of Landsat 8 images: {lc9_ds.n_images}")
    img_names = lc9_ds.collection.aggregate_array('system:index').getInfo()

    lc9_preds = lc9_ds.apply_func(lc9_pred)
    ...
    lc9_fill = lc9_preds.apply_func(fillLc9pred)

    n = lc9_ds.n_images# merged.limit(10).size().getInfo()

    export_list = lc9_fill.collection.toList(n)
