...
# The QA threshold for masking; values between 0.50 and 0.65 generally work well.
...
def s2_qa(img):
    img_rescale = img.divide(10_000)
    return img.select().addBands(img_rescale).updateMask(img.select(QA_BAND).gte(CLEAR_THRESHOLD));
...
def optical_fusion(bbox, start_time, end_time, target_ic):
    ...
    s2_ds_ = hf.Sentinel2(geometry, start_time, end_time, use_qa=False)
    s2_ds_.collection = s2_ds_.collection.linkCollection(csPlus, [QA_BAND])
    s2_ds = s2_ds_.apply_func(s2_qa)

    bands = s2_ds.collection.first().bandNames().getInfo()
    keep_bands = [band for band in bands if 'cs' not in band]
    keep_bands.append('mndwi')
    ...
    logging.info(f"Number of Sentinel 2 images: {s2_ds.n_images}")
    img_names = s2_ds.collection.aggregate_array('system:index').getInfo()

    s2_preds = s2_ds.apply_func(s2_pred)
    ...
    s2_fill = s2_preds.apply_func(fillS2pred)

    n = s2_ds.n_images# merged.limit(10).size().getInfo()

    export_list = s2_fill.collection.toList(n)
